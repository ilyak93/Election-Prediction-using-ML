...
def evaluate_gmm_model(x_train, y_train, model_class, k_fold: int = 3):
    ...
    for train_i, test_i in kf.split(x_train, y_train):
        ...
        jaccard_similarity = metrics.jaccard_score(y_train.values[train_i], _clusters, average='weighted')
...
def test_gmm(x_train: DataFrame, y_train: pd.Series):
    ...
    for model_name, model_class in generate_gmm_models(n_component):
        ...
        k_comparisons = pd.concat([k_comparisons, results])
...
def generative_model_test(data: DataFrame, labels: pd.Series):
...
def print_per_party_distrebution(num_of_clusters, y_pred, y_true: pd.Series):
...
def get_clsuters_to_blocks(x_unlabeled_test, y_unlabeled_test):
    ...
    blocks_dict = {
        0: ["3", "8", "12", "5"],  # from clauster 0
        1: ["4", "9", "2", "11"],  # from cluster 1
        2: ["1", "7", "10"],  # from cluster 3
        3: ["6"],  # from cluster 4
        4: ['0']  # from cluster 2
    }
    ...
    for block, parties in blocks_dict.items():
        ...
        for party in parties:
            block_size += party_counter[int(party)]
        block_size *= 100
...
def get_new_coalition_variance(parties: list, x_unlabeled_test, y_unlabeled_test):
    index = []
    for party in parties:
        prty = int(party)
        index += list(np.argwhere(y_unlabeled_test == prty))
    vars = x_unlabeled_test[selected_numerical_features].values[index].var(axis=0)
    return vars
...
def get_block_center_of_mass(blocks_dict: dict, x_unlabeled_test, y_unlabeled_test):
    ...
    for block, parties in blocks_dict.items():
        index = []
        for party in parties:
            index += list(y_unlabeled_test[y_unlabeled_test == int(party)].index)
